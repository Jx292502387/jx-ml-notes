...
先从最简单的只有一个参数$w$入手，定义$w^* = arg\ \underset{x}{\operatorname{\min}} L(w)$   arg  变元取值
...
- 步骤1：随机选取一个 初始点$w^0$（不一定要随机）
- 步骤2：计算微分，也就是当前的切线斜率，根据斜率来判定移动的方向
	- 大于0向右移动（增加$w$）
	- 小于0向左移动（减少$w$）
- 步骤3：根据学习率移动
- 重复步骤2和步骤3，直到找到最低点![image-20200718195040883](../../../../../%E8%AF%BE%E9%A2%98/%E4%B8%AD%E6%96%87%E5%B0%8F%E8%AE%BA%E6%96%87/image-20200718195040883.png)

![](res/chapter3-8.png)

![image-20200718195147658](../../../../../%E8%AF%BE%E9%A2%98/%E4%B8%AD%E6%96%87%E5%B0%8F%E8%AE%BA%E6%96%87/image-20200718195147658.png)

learning rate ：学习效率

步骤1中，我们随机选取一个 $w^0$，如图8所示，我们有可能会找到**当前的最小值**，并不是**全局的最小值**，这里我们保留这个疑问，后面解决。

解释完**单个模型参数$w$**，引入2个模型参数 $w$ 和 $b$ ， 其实过程是类似的，需要做的是**偏微分**，过程如图9所示，偏微分的求解结果文章后面会有解释，详细的求解过程自行Google。
...
在模型上，我们还可以进一部优化，选择更复杂的模型，使用1元2次方程举例，如图17，发现**训练集求得平均误差为15.4，测试集的平均误差为18.4**
...
## 过拟合问题出现
在模型上，我们再可以**进一部优化，使用更给次方的模型，如图所示**
...
在训练集上面表现更为优秀的模型，为什么在测试集上效果反而变差了？这就是模型在训练集上**过拟合的问题**。

如图所示，每一个模型结果都是一个集合，$5次模型包 \supseteq  4次模型 \supseteq  3次模型$
所以在4次模型里面找到的最佳模型，肯定不会比5次模型里面找到更差   $w_5=0$

![](res/chapter3-21.png)

将错误率结果图形化展示，发现3次方以上的模型，已经出现了过拟合的现象：3次式最佳
...
通过对 Pokemons种类(物种) 判断，将 4个线性模型 合并到一个线性模型中

![](res/chapter3-24.png)

**linear func**

![image-20200718223806927](../../../../../%E8%AF%BE%E9%A2%98/%E4%B8%AD%E6%96%87%E5%B0%8F%E8%AE%BA%E6%96%87/image-20200718223806927.png)
...
**更多特征，更多input**，数据量没有明显增加，仍旧导致**overfitting**
